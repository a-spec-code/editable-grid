    ...
    return {
        select: function (rowIndex, cellIndex) {
            ...
            if (cellIndex < columnRange.topIndex) {
                grid._view.scrollbarX.scrollLeft(grid._view.scrollbarX.scrollLeft() -
                    columns[columnRange.topIndex - 1].width);
            } else if (cellIndex === columnRange.bottomIndex) {
                var totalScrollLeft = 0;
                for (var c = 0; c < columns.length; c++) {
                    totalScrollLeft += columns[c].width;
                }
                grid._view.scrollbarX.scrollLeft(totalScrollLeft);
            } else if (cellIndex > columnRange.bottomIndex) {
                grid._view.scrollbarX.scrollLeft(grid._view.scrollbarX.scrollLeft() +
                    columns[columnRange.bottomIndex + 1].width);
        ...
        getSelectionTd: function () {
            ...
            return grid._view.tbody.find('tr[data-record-id="' + recordId + '"] ' +
