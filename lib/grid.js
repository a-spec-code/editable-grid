...
Grid.prototype = {
    ...
    _sort: function (propertyName) {
        var view = this._view,
            column = _.findWhere(this._options.columns, {id: propertyName});
    ...
    getView: function () {
        ...
        _.each(this._options.columns, function (column) {
            if (column.id !== 'empty-last-column') {
                view.columns.push({
                    id: column.id,
                    width: column.width
                });
    ...
    render: function () {
        ...
        for (var c = columnRange.topIndex; c <= columnRange.bottomIndex; c++) {
            var column = columns[c],
                sortConfig = _.findWhere(view.sortConfig, {name: column.id}),
                styles = [],
                classes = [];
            ...
            tr += '<th class="' + classes.join(' ') + '" data-property-name="' + column.id +
