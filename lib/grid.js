...
var Grid = function (options) {
    ...
    this._els.tableContainer.width(tableWidth);

    // set horizontal scroll bar scroll width
    this._els.scrollbarX.width(tableWidth);
    this._els.scrollbarX.children().eq(0).width(this._options.totalColumnsWidth);
    this._els.scrollbarY.height(TBODY_HEIGHT);
    var scrollHeight = this._stateManager.getRecords().length * this._view.TR_HEIGHT;
    this._els.scrollbarY.children().eq(0).height(scrollHeight);
...
Grid.prototype = {
    ...
    render: function () {
        ...
        var stateManager = this._stateManager,
            ...
            thead = this._els.thead,
            tbody = this._els.tbody,
            view = this._view;
        ...
        var columnRange = calculateRangeOfColumns(view.columnTopIndex,
            columns, this._options.width);

        var recordRange = calculateRangeOfRecords(view, this._els.scrollbarY);
